# ODE model
Base.@kwdef struct SEIRRS4init!{B}
    betafunc::B=mises
end
...
timespan(::SEIRRS4init!) = (-100*365.0 -7.0, -7.0)

function defaultparameters(f::SEIRRS4init!)
    return defaultparameters(SEIRRS4!(betafunc=f.betafunc))
end


function initialstates(f::SEIRRS4init!)
    return initialstates(SEIRRS4!(betafunc=f.betafunc))
end

function solvesettings(::SEIRRS4init!)
    return solvesettings(SEIRRS4!(betafunc=f.betafunc))
end


function (wrapper::SEIRRS4init!)(du, u, p, t)
